...
;(function (window) {
    'use strict';
    // 定义常量
    var EMPTY = '', UNKNOWN = '未知', TYPE_UNDEF = 'undefined', NAME = 'name', VERSION = 'version',
    ...
    var uaRules = {
        ...
        deviceRules:[
            {                                                                           // ZUK mobile
                ...
                patterns:[/android.+;\s(le\s.+)\sbuild\/\w.+mobile/i],
                ...
                defaults:[[MODEL], [VENDOR, 'ZTE'], [TYPE, MOBILE]]
            }, {                                                                        // HTC Nexus 9
                patterns:[/(nexus\s9)/i],
                ...
                patterns:[                                                              // 没有品牌只有型号
                    /android.+;\s(.+)\sbuild\/\w.+mobile/i
                ],
                defaults:[[MODEL], [VENDOR, '未知品牌'], [TYPE, MOBILE]]
            }, {
                patterns:[                                                              // 没有品牌只有型号
                    /android.+;\s(.+)\sbuild\//i
                ],
                defaults:[[MODEL], [VENDOR, '未知品牌'], [TYPE, UNKNOWN]]
            }, {
                patterns:[
        ...
        netTypeRules: [
            {
                patterns:[/(nettype)\/(\w+)/i],                              // WeChat
                defaults:[[NAME, 'NetType'], [VERSION]]
            }
        ],
        tbsRules: [
            {
                patterns:[/(tbs)\/(\d+)/i],                              // WeChat
                defaults:[[NAME, 'TBS'], [VERSION]]
            }
        ]
    };
    // 核心工具
    var tools = {
        ...
        filter:function (type,ua) {
            ...
            switch (type){
                ...
                case 'engine':
                    ...
                    break;
                case 'netType':
                    rules = uaRules.netTypeRules;
                    break;
                case 'tbs':
                    rules = uaRules.tbsRules;
                    break;
            }
            //遍历rules
        ...
        getNetType:function (ua) {
            return this.filter('netType',ua);
        },
        getTBS:function (ua) {
            return this.filter('tbs',ua);
        }
    };
    ...
    var UAFormat = function (uaString) {
        ...
        this.getNetType = function(){
            return tools.getNetType(ua);
        };
        this.getTBS = function(){
            return tools.getTBS(ua);
        };
